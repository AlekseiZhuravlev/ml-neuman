...
class TestPointMeshDistance(unittest.TestCase):
    def setUp(self) -> None:
        np.random.seed(42)
    ...
    def _point_to_tri_distance(point: torch.Tensor, tri: torch.Tensor) -> torch.Tensor:
        ...
        e01_dist, e01_pt = TestPointMeshDistance._point_to_edge_distance(point, tri[[0, 1]])
        e02_dist, e02_pt = TestPointMeshDistance._point_to_edge_distance(point, tri[[0, 2]])
        e12_dist, e12_pt = TestPointMeshDistance._point_to_edge_distance(point, tri[[1, 2]])

        # print('e01_dist', e01_dist, e01_pt)
        # print('e02_dist', e02_dist, e02_pt)
        # print('e12_dist', e12_dist, e12_pt)
        ...
        if inside_tri and (norm > TestPointMeshDistance.eps()):
            ...
            if e01_dist.le(e02_dist) and e01_dist.le(e12_dist):
                # print('batched, 2')
                return e01_dist, e01_pt
            elif e02_dist.le(e01_dist) and e02_dist.le(e12_dist):
                # print('batched, 4')
                return e02_dist, e02_pt
            else:
                # print('batched, 8')
                return e12_dist, e12_pt
    ...
    @staticmethod
    def _is_inside_triangle_batched(point: torch.Tensor, tri: torch.Tensor) -> torch.Tensor:
        """
        Computes whether point is inside triangle tri
        Note that point needs to live in the space spanned by tri = (a, b, c)
        i.e. by taking the projection of an arbitrary point on the space spanned by tri
        ...
        v0 = tri[:, 1] - tri[:, 0]
        v1 = tri[:, 2] - tri[:, 0]
        area = torch.cross(v0, v1).norm(dim=-1) / 2.0

        # check if triangle is a line or a point. In that case, return False
        is_inside_area = area > 1e-5
        # return false if area is too small

        bary = TestPointMeshDistance._point_to_bary_batched(point, tri)

        inside = ((bary >= 0.0) * (bary <= 1.0)).all(dim=-1) * is_inside_area
        return inside

    @staticmethod
    def _point_to_edge_distance_batched(
        point: torch.Tensor, edge: torch.Tensor
    ) -> torch.Tensor:
        """
        Computes the squared euclidean distance of points to edges
        ...
        norm_s01 = torch.einsum('ij,ij->i', s01, s01)

        dist = torch.zeros_like(norm_s01)

        same_edge = norm_s01 < TestPointMeshDistance.eps()
        # dist_same_edge = 0.5 * (point - s0).norm(dim=1)**2 +\
        #                  0.5 * (point - s1).norm(dim=1)**2
        dist_same_edge = 0.5 * torch.einsum('ij,ij->i', point - s0, point - s0) +\
                         0.5 * torch.einsum('ij,ij->i', point - s1, point - s1)

        if same_edge.any():
            dist[same_edge] = dist_same_edge[same_edge]

        t = torch.einsum('ij,ij->i', s01, point - s0) / norm_s01
        t = torch.clamp(t, min=0.0, max=1.0)

        mult = torch.einsum('ij,i->ij', s01, t)
        x = s0 + mult

        dist_other_edge = torch.einsum('ij,ij->i', x - point, x - point)
        dist[~same_edge] = dist_other_edge[~same_edge]

        return dist, x


    @staticmethod
    def _point_to_tri_distance_batched(point: torch.Tensor, tri: torch.Tensor) -> torch.Tensor:
        """
        Computes the squared euclidean distance of points to edges
        Args:
            point: FloatTensor of shape (3)
        ...
        tt = torch.einsum('ij,ij->i', normal, a) - torch.einsum('ij,ij->i', normal, point)

        # multiply by normal
        mult = torch.einsum('ij,i->ij', normal, tt)

        p0 = point + mult
        dist_p = tt * tt

        # Compute the distance of p to all edge segments
        e01_dist, e01_pt = TestPointMeshDistance._point_to_edge_distance_batched(point, tri[:, [0, 1]])
        e02_dist, e02_pt = TestPointMeshDistance._point_to_edge_distance_batched(point, tri[:, [0, 2]])
        e12_dist, e12_pt = TestPointMeshDistance._point_to_edge_distance_batched(point, tri[:, [1, 2]])
        ...
        with torch.no_grad():
            inside_tri = TestPointMeshDistance._is_inside_triangle_batched(p0, tri)

        distances = torch.zeros_like(e01_dist)
        closest_pts = torch.zeros_like(point)

        # inside_tri
        indices_inside = inside_tri & (norm > TestPointMeshDistance.eps())
        distances[indices_inside] = dist_p[indices_inside]
        closest_pts[indices_inside] = p0[indices_inside]

        # e01_dist
        indices_e01 = e01_dist.le(e02_dist) & e01_dist.le(e12_dist) & ~indices_inside
        distances[indices_e01] = e01_dist[indices_e01]
        closest_pts[indices_e01] = e01_pt[indices_e01]

        # e02_dist
        indices_e02 = e02_dist.le(e01_dist) & e02_dist.le(e12_dist) & ~indices_inside & ~indices_e01
        distances[indices_e02] = e02_dist[indices_e02]
        closest_pts[indices_e02] = e02_pt[indices_e02]

        # remaining indices
        rest_indices = ~(indices_inside | indices_e01 | indices_e02)
        distances[rest_indices] = e12_dist[rest_indices]
        closest_pts[rest_indices] = e12_pt[rest_indices]

        # print('batched',
        #     indices_inside.int() * 1 + indices_e01.int() * 2 + indices_e02.int() * 4 + rest_indices.int() * 8,
        # )

        return distances, closest_pts
